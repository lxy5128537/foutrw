...
func init() {
	...
	flag.StringVar(&token, "token", "Ymj5128537", "身份验证令牌（WebSocket Subprotocol）")
	...
	flag.BoolVar(&logToFile, "log", false, "启用日志输出（默认不输出日志）")
}

func main() {
	flag.Parse()

	// Set process name to "xapp"
	setProcessName("xapp")

	// If no parameters provided, use defaults
	if flag.NFlag() == 0 {
		listenAddr = "wss://0.0.0.0:8080"
		token = "Ymj5128537"
		logToFile = false
	}

	// Control log output: default no logs, -log true enables console logging
	if !logToFile {
...
func runWebSocketServer(addr string) {
	...
	// 需求 5：-l 参数不使用路径，服务端功能路径统一为 /xapp
	// （忽略用户传入的路径，强制 /xapp）
	path := "/xapp"
	// 加载入口静态网页（内嵌或同路径 dlxapp.html）
	loadLandingPage()
	...
	// Health check endpoint: /<token> returns 200 (for Uptime Kuma etc.)
	if token != "" {
	...
	// 入口静态网页：访问根路径 / 返回 AI 导航页；其余未匹配路径返回 404
